...
export default (async function ( /* Express App */app, serverConfig) {
  ...
  const connection = await amqp.connect(serverConfig.rabbitMQ.username == null ? `amqp://${serverConfig.rabbitMQ.host}:${serverConfig.rabbitMQ.port}` : `amqp://${serverConfig.rabbitMQ.username}:${serverConfig.rabbitMQ.password}@${serverConfig.rabbitMQ.host}:${serverConfig.rabbitMQ.port}`);
