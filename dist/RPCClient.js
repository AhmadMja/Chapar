...
export default (async function (clientConfig) {
  ...
  const connection = await amqp.connect(clientConfig.rabbitMQ.username == null ? `amqp://${clientConfig.rabbitMQ.host}:${clientConfig.rabbitMQ.port}` : `amqp://${clientConfig.rabbitMQ.username}:${clientConfig.rabbitMQ.password}@${clientConfig.rabbitMQ.host}:${clientConfig.rabbitMQ.port}`);
